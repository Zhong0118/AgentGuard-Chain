...
P1+：CoreCoder guarded 离线回放已可运行，不需要 API key。
...
`agents/miniagent/tools.py` 中的工具分为本地真实工具和本地业务 outbox。它不会真实访问外部业务系统：
...
```text
call_api      生成本地 API 调用记录，同时落盘到 logs/outbox/api_call_log.jsonl
...
P1-16 已把 MiniAgent 的 API、消息和邮件工具从纯内存/固定返回改成文件化 outbox。
...
注意：这仍然不会真实访问外部 API，也不会真实外发消息或邮件。API 记录里 `transport=local_outbox`，消息/邮件记录里 `delivered=false`，表示它们只是本地可审计队列。
...
```text
总记录数：231
MiniAgent：228 条，source=miniagent-scripted，execution_mode=local-tools
...
```text
miniagent-scripted       execution_mode = local-tools
...
| MiniAgent `call_api` | 写入 `logs/outbox/api_call_log.jsonl` | 否 | 本地 API proxy / 受控业务 API |
...
| CoreCoder guarded 离线回放 | `ScriptedCoreCoderLLM` 生成 tool_call | 否 | 离线稳定演示 |
...
## 7. CoreCoder guarded 离线回放

当前已经可以运行无需 API key 的 CoreCoder guarded 离线回放。
...
它已经被 `agents/corecoder_guarded_runner.py` 使用，所以 guarded scripted 离线回放会生效。
...
当前项目已经实现方案 A 的 scripted 离线回放和 real LLM runner 版本。
...
CoreCoder guarded 离线回放：使用脚本化 LLM，不需要 API key。
...
2. 更真实的业务工具接入
   可选：用本地 HTTP server 或文件队列承接 API/message/mail。

3. 运行说明继续补充
   尤其是 CoreCoder guarded 离线回放 / real mode 和 Dashboard 演示流程。
```
...
```text
P1+ 第一步：实现 guarded_corecoder_runner.py（已完成 scripted 离线回放）
P1+ 第二步：准备 CoreCoder 演示样例和日志（已完成 scripted 离线回放）
