...
## 3. 生成 CoreCoder guarded demo 日志
...
scripted demo 不需要 API key，适合稳定演示。
```
...
```text
允许执行的模拟业务工具不会真实外发，而是进入本地 outbox。
这让业务侧效果也能审计。
```
...
如果没有 API key，展示 scripted demo 即可。不要现场强行连真实网络。
...
```text
scripted demo 用于稳定复现 CoreCoder 工具执行前拦截；
...
```text
scripted dataset 用于可复现实验和指标评估，CoreCoder guarded demo 用于展示真实 Agent loop 的适配能力。
```

如果被问为什么不真实外发邮件：

```text
比赛原型需要模拟业务工具和安全审计，不需要真实外发。当前 outbox 设计能保留业务侧证据，同时避免演示环境产生真实外部影响。
```
