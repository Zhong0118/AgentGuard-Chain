...
这个模块的目标是证明：CoreCoder 的真实 agent loop 可以在工具执行前接入
AgentGuard。默认 demo 使用脚本化 LLM，不需要 API key，便于稳定复现。
"""
...
class ScriptedCoreCoderLLM:
    """给 CoreCoder 使用的离线 LLM：第一轮返回工具调用，第二轮返回工具结果。"""
...
def run_corecoder_scripted_demo(
    *,
    ...
    """运行无需 API key 的 CoreCoder guarded demo。"""
    ...
    guarded = GuardedCoreCoderAgent(
        ...
        user_task=f"CoreCoder guarded demo: {demo}",
    ...
    with _pushd(workspace_root):
        response = guarded.chat(f"Run guarded demo: {demo}")
...
def _ensure_openai_import_for_scripted_demo() -> None:
    """离线 scripted demo 不调用 OpenAI，但 CoreCoder llm.py 顶层会导入它。"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run a CoreCoder guarded demo.")
    parser.add_argument(
        "--mode",
        choices=["scripted", "real"],
        default="scripted",
        help="scripted runs offline demos; real uses CoreCoder LLM config from env.",
    )
    parser.add_argument(
        "--demo",
        choices=sorted(DEMO_TOOL_CALLS),
        default="sensitive-file",
        help="Demo tool call generated by the scripted LLM.",
    )
    parser.add_argument(
        "--prompt",
